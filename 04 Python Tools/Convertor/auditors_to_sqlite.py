...
SHEET_NAME = "Sheet1"
DATABASE_FILE = Path(r"C:\CSAI_OS\06 Data\databases\auditors.db")
...
def load_workbook() -> pd.DataFrame:
    ...
    if missing:
        raise ValueError(
            "The auditor workbook is missing required "
            f"column(s): {missing}."
        )

    if "UpdatedAt" in actual_columns:
        raise ValueError(
            "UpdatedAt is generated during synchronization "
            "and must not be present in the source workbook."
        )
    ...
    frame = frame[actual_columns].astype(object)
