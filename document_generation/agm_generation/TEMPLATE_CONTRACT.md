...
`C:\CSAI_OS\document_generation\templates\agm\agm_approve_accounts_template.docx`.
...
`C:\CSAI_OS\document_generation\templates\agm\first_agm_approve_accounts_template.docx`.
It is eligible for Paragraph 15/Third Schedule, Articles 23(b), 34, 36 and 37
of the Company’s Constitution, and Regulation 34 of the Company’s Constitution.
For those authorities it is selected independently for each company when the
literal Python date difference `(financial_year_end - financial_year_start).days`
is less than 364 or greater than 367. Periods from 364 through 367 days inclusive
use the standard template. An explicitly supplied `template_path` or CLI
`--template` always overrides this automatic selection.

The six-page Regulation 90 template is
`C:\CSAI_OS\document_generation\templates\agm\agm_approve_accounts_template_section_90.docx`.
...
Authority-to-File resolutions and does not require an MWR clause. The same
template and complete Section 90 workflow apply to Regulation 90/Table A,
Article 3(d), Clause 53, and Articles 5, 9, 72, 77 and 95 of the Company’s
