...
class TemplateLayoutTests(unittest.TestCase):
    ...
    def test_irregular_template_contains_start_and_end_markers(self) -> None:
        ...
        self.assertNotIn("date of incorporation", paragraphs[0])
...
class LiveDatabaseTests(unittest.TestCase):
    ...
    def test_agensi_is_blocked_by_blank_declarant(self) -> None:
        result = build_document_context("AGENSI SUPREME LOGISTIC SDN. BHD.", self.provider)
        self.assertFalse(result.valid)
        self.assertTrue(
            any(
                issue.code == "missing_fs_field" and "Statutory Declaration" in issue.message
                for issue in result.issues
            ),
            result.issues,
        )
